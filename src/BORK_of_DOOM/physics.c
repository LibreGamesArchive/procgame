                     ...
                     vec3 out, vec3 pos, vec3 size)
{
    ...
    for(x = check[0][0]; x < check[1][0]; ++x) {
        for(y = check[0][1]; y < check[1][1]; ++y) {
            for(z = check[0][2]; z < check[1][2]; ++z) {
                ...
                mat4 transform;
                mat4_translate(transform, area_pos[0], area_pos[1], area_pos[2]);
                ...
                *coll_out = (struct bork_collision) {
                ...
                vec3_dup(push, tile_push);
                vec3_dup(coll_out->norm, tile_norm);
                ++count;
            }
        }
    }
    vec3 pos_;
    vec3_scale(pos_, pos, 0.5);
    enum bork_area area = bork_map_get_area(map, (int)pos_[0], (int)pos_[1], (int)pos_[2]);
    if(area < BORK_AREA_EXTERIOR) {
        ...
        ARR_FOREACH_PTR(map->objects[area], obj, i) {
            ...
            int c = pg_model_collide_ellipsoid(&map->door_model, model_transform,
            ...
            *coll_out = (struct bork_collision) {
            ...
            vec3_dup(push, obj_push);
            vec3_dup(coll_out->norm, obj_norm);
            ++count;
        }
    }
